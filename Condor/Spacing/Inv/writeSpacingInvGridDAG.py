...
# Here the set of spacings between two mutations in a chromosome with an inversion

import numpy as np

submitFile = 'SAIsim_TwoSpacingInv.sub'
...
with open('SAIsim_TwoSpacingInvGrid.dag','w') as outfile:
	...
	for spacing in np.around(np.linspace(0.0,0.65,num=53),8):
		...
		for batch in np.arange(numBatches):
			jobName = 'TSI_'+'0'*(4-len(str(int(spacing*10000))))+str(int(spacing*10000))+"_"+str(batch)
			# print("Job "+jobName+":\n\tspacing\t"+str(spacing))
